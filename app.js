...
function dayStatusInfo(){
  ...
  if(alerts > 0){
    return {
      ...
      message:`Houve ${alerts} alerta(s) de glicemia hoje. Verifique a Dona Nair.`,
      done, pending, late, alerts
    };
  }
  if(late > 0){
    return {
      ...
      icon:'🔴',
      title:'Atenção necessária',
      message:`Existem ${late} atividade(s) atrasada(s).`,
      done, pending, late, alerts
    };
  }
  if(pending > 0){
    return {
      ...
      icon:'🟡',
      title:'Há pendências',
      message:`Faltam ${pending} atividade(s) para concluir o dia.`,
      done, pending, late, alerts
    };
  }
  return {
    ...
    icon:'🟢',
    title:'Tudo em dia',
    message:'Dona Nair está com a rotina de hoje concluída.',
    done, pending, late, alerts
  };
}

function renderSummary(){
  const info = dayStatusInfo();
  $('#countDone').textContent = info.done;
  $('#countPending').textContent = info.pending;
  $('#countLate').textContent = info.late;
  $('#countAlerts').textContent = info.alerts;
  const box = $('#dayHealth');
  box.className = 'status-banner ' + info.level;
  box.innerHTML = `<div class="status-icon">${info.icon}</div><div><strong>${info.title}</strong><span>${info.message}</span></div>`;
}
...
function renderFamily(){
  const info = dayStatusInfo();
  const top = $('#familyTopStatus');
  top.className = 'family-top-status ' + info.level;
  top.innerHTML = `<div class="family-big-icon">${info.icon}</div><div><strong>${info.title}</strong><span>${info.message}</span></div>`;
